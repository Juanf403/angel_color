...
<?php
			if ( isset($_GET['del']) ){
				...
				mysql_query("DELETE FROM servicios WHERE idservicios='".$del."'");
			}

			if ( isset($_GET['buscar']) ){
				$buscar = mysql_real_escape_string($_GET['buscar']);
				$consulta  = "SELECT * FROM servicios WHERE 
					(servicio LIKE '%".$buscar."%' OR 
						tipo LIKE '%".$buscar."%' OR 
						precio LIKE '%".$buscar."%') 
					ORDER BY servicio ASC";
					$url = "admin.php?m=servicios&buscar=".$buscar;
			} else {
				$consulta  = "SELECT * FROM servicios ORDER BY servicio ASC";
				$url = "admin.php?m=servicios";
			}
